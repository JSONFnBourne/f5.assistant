...
def expand_tables_to_lists(soup: BeautifulSoup) -> None:
    for table in list(soup.find_all("table")):
        ...
        data_rows = rows
        first_row = rows[0]
        header_candidate = table.find("tr")
        if header_candidate and header_candidate.find_all("th"):
            headers = first_row
            data_rows = rows[1:]
        if not data_rows:
            continue
        ...
        for cells in data_rows:
            ...
            if headers and len(headers) == len(cells):
                ...
                entries.append(" | ".join(filter(None, cells)))
            if entries:
                bullet_lines.append(" | ".join(entries))
        if not bullet_lines:
            continue
        ul = soup.new_tag("ul")
...
def run_clean(args: CleanArgs) -> None:
    ...
    for path, record in yield_raw_documents(args.raw_dir):
        ...
        expand_tables_to_lists(content_soup)
