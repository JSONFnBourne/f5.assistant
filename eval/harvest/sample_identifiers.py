...
def main() -> None:
    ...
    kb = sample(
        c.execute(
            "SELECT doc_id,title FROM documents WHERE source='f5_kb' AND doc_id LIKE 'K%'"
        ).fetchall(),
        5,
        rnd,
    )
    sec = sample(
        c.execute(
            "SELECT doc_id,title FROM documents WHERE source='f5_security' AND doc_id LIKE 'K%'"
        ).fetchall(),
        3,
        rnd,
    )
    for did, _title in kb + sec:
        add(
            f"What does F5 article {did} address?",
            did,
            "k-number",
    ...
    cve_rows = c.execute(
        ...
        "WHERE keywords LIKE '%CVE-%' ORDER BY (source='f5_security') DESC"
    ).fetchall()
    ...
    for cve, did in sample(picks, 3, rnd):
        add(
            f"Which F5 article documents {cve}?",
            did,
            "cve",
    ...
    iru = sample(
        c.execute(
            "SELECT doc_id,title FROM documents WHERE source='irules' AND title LIKE '%::%'"
        ).fetchall(),
        3,
        rnd,
    )
    for did, title in iru:
        add(
            f"What does the {title} iRule command do?",
            did,
            "irule",
            "identifier-class; tautological gold=sampled doc_id; seed=42",
        )
    ...
    rfc = sample(
        c.execute(
            "SELECT doc_id,title FROM documents WHERE source='rfc' AND doc_id LIKE 'rfc%'"
        ).fetchall(),
        2,
        rnd,
    )
